...
.. code-block:: console
   ...
   Put the results in a stacked bar graph, with on the time on the X-axis and the load on the Y-axis. The base load should be at the bottom in one color while all the deployments should be stacked on top of that with a different color for each deployment.
